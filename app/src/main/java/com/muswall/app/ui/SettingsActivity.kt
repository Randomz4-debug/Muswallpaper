...
class SettingsActivity : AppCompatActivity() {
    ...
    private val customPhotoPicker = registerForActivityResult(
        ...
        prefs.customPhotoUri = uri.toString()
        prefs.photoSource = PreferencesManager.PHOTO_CUSTOM
        updatePhotoLabels()
        ToastCompat.show(this, "Custom music photo selected")
    }

    private val originalWallpaperPicker = registerForActivityResult(
        ActivityResultContracts.OpenDocument()
    ) { uri ->
        if (uri == null) return@registerForActivityResult
        try {
        ...
        ioScope.launch {
            ...
            ToastCompat.show(
                ...
                if (ok) "Original $target wallpaper saved" else "Could not save wallpaper"
            )
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.restoreHomeRow).setOnClickListener {
            chooseOriginalWallpaper(WallpaperManager.FLAG_SYSTEM)
        }
        findViewById<TextView>(R.id.restoreLockRow).setOnClickListener {
            chooseOriginalWallpaper(WallpaperManager.FLAG_LOCK)
        }
        findViewById<TextView>(R.id.timingRow).setOnClickListener { chooseTiming() }
        findViewById<TextView>(R.id.permissionsRow).setOnClickListener {
            XiaomiHelper.openNotificationListenerSettings(this)
        }
        findViewById<TextView>(R.id.serviceRow).setOnClickListener {
            XiaomiHelper.openNotificationListenerSettings(this)
        }
        findViewById<TextView>(R.id.compatibilityRow).setOnClickListener {
            ToastCompat.show(this, "Compatibility mode uses the fast live-wallpaper renderer.")
        }
        ...
        findViewById<TextView>(R.id.faqRow).setOnClickListener {
            AlertDialog.Builder(this)
                .setTitle("MusWall FAQ")
                .setMessage(
                    "Choose Automatic for the most reliable artwork detection, or select Album Art, " +
                        "Track Art, Display Icon, Notification Art or a Custom Image. Lyrics can be " +
                        "shown when the player exposes lyric metadata."
                )
        ...
        findViewById<TextView>(R.id.customPhotoRow).setOnClickListener {
            customPhotoPicker.launch(arrayOf("image/*"))
        }
        ...
        findViewById<SwitchCompat>(R.id.effectsRestore).apply {
            ...
            setOnCheckedChangeListener { _, value ->
                if (!value) prefs.effect = PreferencesManager.EFFECT_BLUR
            }
        }
    ...
    private fun choosePhotoSource() {
        val labels = arrayOf(
            "Automatic — try all available sources",
            "Album artwork — standard album cover",
            "Track artwork — player ART field",
            "Display icon — player display icon",
            "Notification artwork — notification large icon",
            "Custom image — one image for every track"
        )
        val values = arrayOf(
            PreferencesManager.PHOTO_AUTO,
            PreferencesManager.PHOTO_ALBUM,
            PreferencesManager.PHOTO_ART,
            PreferencesManager.PHOTO_DISPLAY_ICON,
            PreferencesManager.PHOTO_NOTIFICATION,
            PreferencesManager.PHOTO_CUSTOM
        )
        AlertDialog.Builder(this)
            .setTitle("Which music photo should be used?")
            .setItems(labels) { _, index ->
                if (values[index] == PreferencesManager.PHOTO_CUSTOM && prefs.customPhotoUri.isBlank()) {
                    customPhotoPicker.launch(arrayOf("image/*"))
                } else {
                    prefs.photoSource = values[index]
                    updatePhotoLabels()
                    ToastCompat.show(this, "Music photo source saved")
                }
            }
            .show()
    }
    ...
    private fun chooseResolution() {
        val labels = arrayOf(
            "Device native — recommended for live wallpaper",
            "1080p — 1920 × 1080",
            "1440p — 2560 × 1440",
            "4K UHD — 3840 × 2160",
            "8K UHD — 7680 × 4320",
            "12K — 11520 × 6480",
            "16K — 15360 × 8640",
            "Custom — choose exact pixels"
        )
        val values = arrayOf(
            PreferencesManager.RESOLUTION_DEVICE,
            PreferencesManager.RESOLUTION_1080P,
            PreferencesManager.RESOLUTION_1440P,
            PreferencesManager.RESOLUTION_4K,
            PreferencesManager.RESOLUTION_8K,
            PreferencesManager.RESOLUTION_12K,
            PreferencesManager.RESOLUTION_16K,
            PreferencesManager.RESOLUTION_CUSTOM
        )
        AlertDialog.Builder(this)
            .setTitle("Wallpaper render resolution")
            .setMessage("Higher resolutions create larger files and use much more RAM. Device native is recommended for live wallpaper.")
            .setItems(labels) { _, index ->
                if (values[index] == PreferencesManager.RESOLUTION_CUSTOM) {
                    chooseCustomResolution()
                } else {
                    prefs.renderResolution = values[index]
                    updateResolutionLabel()
                    ToastCompat.show(this, "Resolution saved")
                }
            }
            .show()
    }
    ...
    private fun chooseCustomResolution() {
        ...
        val width = android.widget.EditText(this).apply {
            hint = "Width, e.g. 7680"
            inputType = android.text.InputType.TYPE_CLASS_NUMBER
            setText(prefs.customRenderWidth.toString())
        }
        val height = android.widget.EditText(this).apply {
            hint = "Height, e.g. 4320"
            inputType = android.text.InputType.TYPE_CLASS_NUMBER
            setText(prefs.customRenderHeight.toString())
        }
        view.addView(width)
        view.addView(height)
        AlertDialog.Builder(this)
            .setTitle("Custom render resolution")
            .setMessage("Maximum 16384 × 16384. Very large images may exceed your phone's RAM.")
            .setView(view)
            .setNegativeButton("Cancel", null)
            .setPositiveButton("Save") { _, _ ->
                val w = width.text.toString().toIntOrNull()?.coerceIn(160, 16384) ?: 1080
                val h = height.text.toString().toIntOrNull()?.coerceIn(240, 16384) ?: 2400
                prefs.customRenderWidth = w
                prefs.customRenderHeight = h
                prefs.renderResolution = PreferencesManager.RESOLUTION_CUSTOM
                updateResolutionLabel()
            }
            .show()
    }
    ...
    private fun chooseBackgroundMode() {
        AlertDialog.Builder(this)
            .setTitle("Wallpaper background")
            .setItems(arrayOf("Album artwork (blurred)", "Solid color", "Gradient", "Auto color from album")) { _, which ->
    ...
    private fun chooseColor(second: Boolean) {
        ...
        AlertDialog.Builder(this)
            .setTitle(if (second) "Gradient end color" else "Background color")
    ...
    private fun chooseAccentColor() {
        ...
        AlertDialog.Builder(this)
            .setTitle("Cover accent")
            .setItems(colors) { _, which -> prefs.accentColor = colors[which] }
            .show()
    }

    private fun chooseOriginalWallpaper(which: Int) {
        originalTarget = which
        originalWallpaperPicker.launch(arrayOf("image/*"))
    }

    private fun chooseTiming() {
        AlertDialog.Builder(this)
            .setTitle("Restore timing")
            .setItems(arrayOf("Immediately", "5 seconds", "10 seconds", "30 seconds")) { _, which ->
                prefs.restoreDelay = intArrayOf(0, 5, 10, 30)[which]
            }
            .show()
    }
